...
def zipper_interp_cols(image, mask, interp_mask, **kwargs):
   ...
   :Optional parameters (passed as **kwargs)
    ...
    if region_file:
        msg = f"Will write ds9 regions to {region_file}"
    ...
    msg += f"with xblock={xblock}, yblock={yblock}, maxcols={max_cols}, mincols={min_cols}, add_noise={add_noise}, ydilate={ydilate}"
    if logger:
        logger.info(msg)
    else:
        print("#", msg)
    ...
    for run in range(len(xstart)):
        ...
        if n_good > 0:
            ...
            msg = f"WARNING: All sampling values equal to zero on slices [{y1a}:{y1b},{x1}:{x2}] and [{y2a}:{y2b},{x1}:{x2}]"
            if logger:
                logger.info(msg)
            else:
                print("#", msg)
        ...
        if region_file:
            reg.write(f"line {x0 + 1} {y1 + 1} {x0 + 1} {y2 + 1}\n")
